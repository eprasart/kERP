...
namespace kERP
{
    public partial class frmUnitMeasure : Form
    {
        ...
        frmMsg fMsg = null;
        string ModuleName = "IC Unit of Measure";
        string TitleLabel = UnitMeasureFacade.TitleLabel;
        ...
        {
            var valid = new Validator(this, "ic_unit_measure");
            string sCode = txtCode.Text.Trim();
            if (sCode.Length == 0)
                valid.Add(txtCode, "code_blank");
            else if (UnitMeasureFacade.Exists(sCode, Id))
                valid.Add(txtCode, "code_exists");
            if (txtDescription.IsEmptyTrim) valid.Add(txtDescription, "description_blank");
            if (!Util.IsDouble(txtFactor.Text)) valid.Add(txtFactor, "default_factor_invalid");
            return valid.Show();
        }
        ...
        private void frmUnitMeasure_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    UnitMeasureFacade.SetStatus(Id, Constant.RecordStatus_Deleted);
                }
                catch (Exception ex)
                {
